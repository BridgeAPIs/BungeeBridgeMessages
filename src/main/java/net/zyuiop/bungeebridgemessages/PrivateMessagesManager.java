...
import net.bridgesapis.bungeebridge.services.Service;
...
public class PrivateMessagesManager {
	...
	private BungeeBridgeMessagesPlugin plugin;
	private HashMap<UUID, UUID> lastMessageSender = new HashMap<>();

	public PrivateMessagesManager(BungeeBridgeMessagesPlugin plugin) {
		this.plugin = plugin;
	}
	...
	public void send(ProxiedPlayer sender, String name, UUID id, String message) {
		if (ServiceManager.hasService(IChatListener.class)) {
			IChatListener chatListener = ServiceManager.getService(IChatListener.class);
			if (chatListener.isMuted(sender.getUniqueId())) {
				// TODO : Translate mute message
				sender.sendMessage(new ComponentBuilder("Vous êtes actuellement muet pour une durée de " + Misc.formatTime((chatListener.getEnd(sender.getUniqueId()).getTime() - System.currentTimeMillis()) / 1000)).color(ChatColor.RED).create());
				sender.sendMessage(new ComponentBuilder("Raison : " + ChatColor.YELLOW + chatListener.getReason(sender.getUniqueId())).color(ChatColor.RED).create());
				return;
			}
		}
		...
		boolean bypass = (ServiceManager.hasService(IFriendsManagement.class) && ServiceManager.getService(IFriendsManagement.class).isFriend(sender.getUniqueId(), id));
